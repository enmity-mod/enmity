...
export default function ({ type }: { type: "plugin" | "theme"; }) {
	...
	React.useEffect(() => {
		...
		(isPlugin ? Plugins : Themes).on('loaded', onChange);
		(isPlugin ? Plugins : Themes).on('unloaded', onChange);
		(isPlugin ? Plugins : Themes).on('uninstalled', onChange);

		return () => {
			(isPlugin ? Plugins : Themes).off('installed', onChange);
			(isPlugin ? Plugins : Themes).off('loaded', onChange);
			(isPlugin ? Plugins : Themes).off('unloaded', onChange);
